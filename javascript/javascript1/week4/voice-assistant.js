...
function getReply(command) {
    ...
    if (commandArray[0].toLowerCase() === 'hello') {
        if (name === '') {
    ...
    else if (commandArray[0].toLowerCase() === 'what') {
...
function getMathResult(commandArray) {
    ...
    const a = parseInt(commandArray[2]);
    const b = parseInt(commandArray[4]);
...
function setTimer(commandArray) {
    ...
    let myVar = setInterval(function () { myStopFunction(); }, 1000 * timeInSeconds);
...
function getDate() {
    ...
    return (toDate.getDate() + '. of ' + months[toDate.getMonth()] + ' ' + toDate.getFullYear());
}
